...
for i = 1:(scenarioData.StopTime/scenarioData.SampleTime)
    ...
    if mod(i, 10) == 1
        ...
        plot(clusterer,objectData(:, 1:2),clustererPlot,'Parent',axesHandle);
        xlim(axesHandle, [-15 15]);
        ylim(axesHandle, [-10 50]);
     
        % Get the unique cluster labels
        clusterLabels = unique(clustererPlot);

        % Remove previous rectangles
        oldRectangles = findobj('Parent',axesHandle,'Type', 'rectangle');
        ...
        for j = 1:length(clusterLabels)            
            ...
            rectangle('Parent', axesHandle, 'Position',[minX minY maxX-minX maxY-minY],'EdgeColor','r');
        end
    end
